from django.conf import settings
from django.conf.urls.static import static
from django.contrib import admin
from django.contrib.staticfiles.storage import staticfiles_storage
from django.urls import include, re_path
from django.views import defaults as default_views
from django.views.generic.base import RedirectView

from core.views import index, protected_serve

from onetimelink import presettings

admin.site.site_header = 'MyTemplate Administration'
admin.site.index_title = 'MyTemplate'
admin.site.site_title = 'Admin'

urlpatterns = [
    re_path(r'^favicon.ico', RedirectView.as_view(url=staticfiles_storage.url('img/favicon.ico'))),

    re_path(settings.ADMIN_URL, admin.site.urls),

    re_path(r'^accounts/', include('allauth.urls')),

    re_path(r'^%s/' % presettings.DYNAMIC_LINK_URL_BASE_COMPONENT, include('onetimelink.urls')),

    re_path(r'^$', index, name='home'),

    re_path(r'^api/v1/', include('core.v1_urls', namespace='v1')),
    re_path(r'^api/v2/', include('core.v2_urls', namespace='v2')),
]

if settings.FORCE_SCRIPT_NAME:
    media_url = settings.MEDIA_URL.replace(settings.FORCE_SCRIPT_NAME, '')[1:]
    static_url = settings.STATIC_URL.replace(settings.FORCE_SCRIPT_NAME, '')[1:]
else:
    media_url = settings.MEDIA_URL[1:]
    static_url = settings.STATIC_URL[1:]
if settings.DEBUG:
    urlpatterns += static(media_url, document_root=settings.MEDIA_ROOT)
    urlpatterns += static(static_url, document_root=settings.STATIC_ROOT)
else:
    urlpatterns += [re_path(rf'^{media_url}(?P<path>.*)$', protected_serve, {'document_root': settings.MEDIA_ROOT}, name='protected_serve_media')]
    urlpatterns += [re_path(rf'^{static_url}(?P<path>.*)$', protected_serve, {'document_root': settings.STATIC_ROOT}, name='protected_serve_static')]

if settings.DEBUG:
    # This allows the error pages to be debugged during development, just visit
    # these url in browser to see how these error pages look like.
    urlpatterns += [
        re_path(r'^400/$', default_views.bad_request, kwargs={'exception': Exception('Bad Request!')}),
        re_path(r'^403/$', default_views.permission_denied, kwargs={'exception': Exception('Permission Denied')}),
        re_path(r'^404/$', default_views.page_not_found, kwargs={'exception': Exception('Page not Found')}),
        re_path(r'^500/$', default_views.server_error),
    ]
    if 'debug_toolbar' in settings.INSTALLED_APPS:
        import debug_toolbar
        urlpatterns = [
            re_path(r'^__debug__/', include(debug_toolbar.urls)),
        ] + urlpatterns
